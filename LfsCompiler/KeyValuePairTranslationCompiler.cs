                ...
                using (var writer = new StreamWriter(File.Create(this.GetPathToBuildOutputForLocale(locale))))
                {
                    ...
                    foreach (var key in keysInOrder)
                    {
                        ...
                        if (sourceStringToTranslationsMap.TryGetValue(sourceString, out var translation))
                        {
                            ...
                            {
                                writer.WriteLine($"    // >>>MACHINE GENERATED by {translation.author} on {translation:d}");
                            }
                            else
                            {
                                writer.WriteLine($"    // Translated by {translation.author} on {translation:d}");
                            }
                        ...
                        else
                        {
                            ...
                            if (closeMatch is not null)
                            {
                                ...
                                writer.WriteLine($"    // >>>SOURCE STRING CHANGED - originally translated by {translation.author} on {translation:d}");
